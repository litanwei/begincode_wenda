...
public class ProblemHandler {
	...
	/**
	 * 添加问题 一个问题关联着 标签表(传入标签名集合) 消息表(如果传入的userId不为空 则存入消息表中) 问题和标签对应的表
	 *
	 * @param problem
	 *            前台传入的问题
	 * @param label
	...
	public void addProblem(Problem problem, Label label) {
		...
		messageService.createMessage(problem.getProblemId(), null,
				begincodeUserService.contentFilter(problem.getContent()));
		// 发送http请求给搜索端
	...
	/**
	 * 传入的labelName集合 判断是否有存在 如果存在加入集合 不存在先加入数据库 再加入集合
	 *
	 * @param labelNameSet
	 * @return 集合标签
	 */
